...
function Interface(props: Props) {
  const fileExport = async () => {
    ...
    const fh = await window.showSaveFilePicker({
      suggestedName: props.info.track.toString().padStart(3, "0") + ".json",
    });
  ...
  return (
    <>
      <div className="interface">
        <button className="waves-effect waves-light btn" onClick={() => fileExport()}>
          EXPORT
        </button>
        <button onClick={() => fileImport()} className="waves-effect waves-light btn">
          IMPORT
        </button>
      </div>
    </>
